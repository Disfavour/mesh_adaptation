...
def solve_test_problem(mesh_fname, d=0.1):
    ...
    a = 1
    #d = 0.1 / 4
    #f = a / d * ufl.exp(- (x - x_0) ** 2 / d ** 2

    #f = dolfinx.fem.Constant(domain, dolfinx.default_scalar_type(-6))

    x = ufl.SpatialCoordinate(domain)
    x_0 = ufl.as_vector([-1, -0.5])
    f =  a/d * ufl.exp(-(x-x_0)**2 / d**2)
    ...
    weight_function_ufl = 0.001 + ufl.sqrt(ufl.dot(ufl.grad(u), ufl.grad(u))) ** 2
...
def iterative_adaptation():
    n = 40
    niter = 100
    # (0.1/4, 0.1/2, 0.1, 0.1*2, 0.1*4)
    for j, d in enumerate((0.1/4, 0.1/2, 0.1, 0.1*2, 0.1*4), 1):
        fname_mesh = f'meshes/iterative_adaptation_3_{j}'
        fname_images = f'images/iterative_adaptation_3_{j}'
        ...
        for i in range(1, 5 + 1):
            ...
            points, triangles, values_weight, values_solution = solve_test_problem(previous_mesh, d)
